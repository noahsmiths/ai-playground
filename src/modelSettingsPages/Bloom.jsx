    ...
    return (
        <>
            ...
            <div className="flex flex-row items-end">
                ...
                <input type="checkbox" checked={maxLengthEnabled} className="toggle toggle-accent mb-2 ml-1" id="bloom-max-len-toggle" onChange={(e) => { updateSingleSetting({maxLengthEnabled: e.target.checked}); }}/>
            </div>
            <div className="flex flex-row items-center gap-4">
                <input type="range" min="1" max="250" value={maxLength} className="range range-primary w-5/6 disabled:opacity-25" step="1" id="bloom-max-len" onChange={(e) => { updateSingleSetting({ maxLength: +e.target.value }); }} disabled={!maxLengthEnabled} /><span className="input w-max h-min p-2 bg-base-200">{maxLength}</span>
            </div>
            <div className="flex flex-row items-end">
                <label className="label" htmlFor="bloom-top-p-toggle">
                    <span className="label-text-lg pt-4">Top P</span>
                </label>
                <input type="checkbox" checked={topPEnabled} className="toggle toggle-accent mb-2 ml-1" id="bloom-top-p-toggle" onChange={(e) => { updateSingleSetting({topPEnabled: e.target.checked}); }} />
            </div>
            <div className="flex flex-row items-center gap-4">
                <input type="range" min="0" max="1" value={topP} className="range range-primary w-5/6 disabled:opacity-25" step="0.01" id="bloom-top-p" onChange={(e) => { updateSingleSetting({ topP: +e.target.value }); }} disabled={!topPEnabled} /><span className="input w-max h-min p-2 bg-base-200">{topP}</span>
            </div>
            <div className="flex flex-row items-end">
                <label className="label" htmlFor="bloom-rep-pen-toggle">
                    <span className="label-text-lg pt-4">Repetition Penalty</span>
                </label>
                <input type="checkbox" checked={repetitionPenaltyEnabled} className="toggle toggle-accent mb-2 ml-1" id="bloom-rep-pen-toggle" onChange={(e) => { updateSingleSetting({repetitionPenaltyEnabled: e.target.checked}); }} />
            </div>
            <div className="flex flex-row items-center gap-4">
                <input type="range" min="0" max="100" value={repetitionPenalty} className="range range-primary w-5/6 disabled:opacity-25" step="0.01" id="bloom-rep-pen" onChange={(e) => { updateSingleSetting({repetitionPenalty: +e.target.value}); }} disabled={!repetitionPenaltyEnabled} /><span className="input w-max h-min p-2 bg-base-200">{repetitionPenalty}</span>
            </div>
            <div className="flex flex-row items-end gap-1">
                ...
                <input type="checkbox" checked={doSample} className="toggle toggle-accent mb-2" id="bloom-do-sample-toggle" onChange={(e) => { updateSingleSetting({doSample: e.target.checked}); }} />
            </div>
        </>
    )
}
